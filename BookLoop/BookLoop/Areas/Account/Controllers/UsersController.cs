...
namespace Account.Controllers
{
	...
	public class UsersController : AccountAreaController
	{
		...
		// ===== 詳細 =====
		// 也可用獨立權限 Users.Details；目前沿用 Users.Index 即可瀏覽
		[Authorize(Policy = "Users.Index")]
		...
		public async Task<IActionResult> Create(User input)
		{
			...
			{
				ModelState.AddModelError(nameof(input.Email), "新增失敗，請確認 Email 是否重複。");
				FillUserTypeStatus();
				return View(input);
			}
		}
		...
		public async Task<IActionResult> Edit(int id, User input)
		{
			...
			{
				ModelState.AddModelError(nameof(input.Email), "儲存失敗，請確認 Email 是否重複。");
				FillUserTypeStatus();
				return View(entity);
			}
		}
