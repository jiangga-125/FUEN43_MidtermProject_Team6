...
namespace BookLoop.Services
{
    ...
    public class ReservationQueueService
    {
        ...
        private const int DefaultBorrowDays = 3;
        public ReservationQueueService(BorrowContext db) => _db = db;
        ...
        public async Task<bool> PromoteNextReservationAsync(int listingId, CancellationToken ct = default)
        {
            ...
            //// 設定可取書時間 & 逾期日
            next.ReadyAt = now.AddDays(1).AddHours(8);//設定歸還後1天8點開始可取書
            next.ExpiresAt = now.AddDays(3).AddHours(9).AddMinutes(30);//設定逾期日為可借閱後三天的17:30            
