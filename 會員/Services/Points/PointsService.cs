...
namespace 會員.Services.Points
{
	public class PointsService : IPointsService
	{
		...
		{
			var mp = await _db.MemberPoints.AsNoTracking().FirstOrDefaultAsync(x => x.MemberId == memberId);
		...
		{
			if (points <= 0) return Result<bool>.Fail("扣點必須為正數");

			var mp = await _db.MemberPoints.FirstOrDefaultAsync(x => x.MemberId == memberId);
